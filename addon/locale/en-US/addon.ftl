startup-begin = LLM Reader is loading
startup-finish = LLM Reader is ready
menu-generate = LLM: Generate HTML and Open
panel-loading = Generating view...
panel-ready = Ready
panel-error = Generation failed
panel-button-analyze = Analyze
    .tooltiptext = Analyze paper with AI
panel-button-open = Open HTML
panel-scope-full = Full text
panel-scope-highlights = Highlights
panel-scope-meta = Metadata only
prefs-title = LLM Reader
tabpanel-reader-tab-label =
    .label = LLM Reader
tab-loading-title = Analyzing Paper...
tab-loading-subtitle = LLM is generating an interactive summary
panel-click-to-analyze = Click the analyze button to start AI analysis
panel-analyzing = AI is analyzing...

# Chat panel
tabpanel-chat-tab-label =
    .label = Chat
panel-chat-ready = Ready to chat
panel-button-clear-chat =
    .tooltiptext = Clear chat history
panel-no-selection = No text selected in PDF
panel-chat-with-selection = Chat about selection
